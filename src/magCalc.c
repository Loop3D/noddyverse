...
double poly_interp(givenx, giveny, nxy, wantx)
	double *givenx, *giveny;int nxy;double wantx; {
	double wanty, temp;
	int i, j;
...
void upwd_continue(mareal, maimg, real, img, Z, vpxy, uvSqrtArray)
	double *mareal, *maimg, *real, *img, Z;int vpxy;double *uvSqrtArray; {
	int i, xy;
...
static void
#if XVT_CC_PROTO
CSIROWriteMap(int xmax, int ymax, int xsize, double *master, FILE *fname,
...
#endif
{
	...
	fprintf(fname, "%d\t%d\t%d\t%d\n", calcRangeInCubes, xmax + 1, ymax + 1,
			zmax);
...
static int
#if XVT_CC_PROTO
CSIROfillAnomImage(double *result, int fileType, int nx, int ny, int xsize,
...
#endif
{
	...
	for (y = 1; y < ny; y++) /* was filterSize-1 */
	{
		...
		{
			anomImage->data[yIndex][xIndex] =
					(double) (result[xy++] * unitScale);
			xIndex++;
		}
		yIndex++;
	}
...
void rebin_vec(invec, outvec, insiz, outsiz, inoff, outoff)
	double *invec, *outvec;int insiz, outsiz, inoff, outoff; {
	register int i, ii;
	int j = insiz - 1;
...
void resample_columns(array, buffer, buffer2, ncol, totcol, iny, outy)
	double *array, *buffer, *buffer2;int ncol, totcol, iny, outy; {
	int i, j;
	double *a, *ptr;
...
void ramp_interp(start, end, interp, npts, span)
	double start, end, *interp;int npts, span; {
	int i;
	double v, delta, *ptr;
...
void reflect(data, refdata, npts, mpts, span, sign)
	double *data, *refdata;int npts, mpts, span;double sign; {
	int i, j;
	double *dptr, *ptr;
...
void up_back_add(Z, real, imag, result, upbreal, upbimg, ht_index, npoints, vpx,
		vpy, vpxy, uvSqrtArray)
	double Z, *real, *imag, *result, *upbreal, *upbimg;HT_INDEX *ht_index;int npoints;int
			vpx, vpy, vpxy;double *uvSqrtArray; {
	...
	for (i = 0; i < npoints; i++) {
		if (Z2 != hi->height) {
			Z2 = hi->height;
			upwd_continue(real, imag, upbreal, upbimg, fabs(Z2 - Z), vpxy,
					uvSqrtArray);
...
int grab_line(unit, line, arraySize)
	FILE *unit;char *line;int arraySize; {
	int l = 0;
	char c;
	...
	line[l] = 0;
	read_loop: fread((void*) &c, 1, 1, unit);
	if (feof(unit) || (c == '\n'))
		return (l);
	line[l++] = c;
	line[l] = 0;
...
int CSIROgetFieldAtLocation(geoFizz, xLoc, yLoc, zLoc, inclination, declination,
		intensity)
	GEOPHYSICS_OPTIONS *geoFizz;double xLoc, yLoc, zLoc, *inclination,
...
int magCalc(outputFilename, blockName, numLayers, layerDimensions, numCubeSizes,
		cubeSizes, viewOptions, options, xyzCalc, pointPos, numPoints,
		xyzImportFile, densityCalc, susCalc, remCalc, aniCalc, indexCalc,
		numProps, propArray, indexData, densityData, magSusData, remSusDecData,
		remSusAziData, remSusStrData, aniSusDipData, aniSusDDirData,
		aniSusPitchData, aniSusAxis1Data, aniSusAxis2Data, aniSusAxis3Data,
		magImage, grvImage)
	char *outputFilename, *blockName;int numLayers, **layerDimensions,
			numCubeSizes, *cubeSizes;BLOCK_VIEW_OPTIONS *viewOptions;GEOPHYSICS_OPTIONS *options;int xyzCalc;double **pointPos;int numPoints;FILE_SPEC *xyzImportFile;int
			densityCalc, susCalc, remCalc, aniCalc, indexCalc, numProps;LAYER_PROPERTIES **propArray;short ***indexData;float
			...
			***aniSusAxis3Data;DOUBLE_2D_IMAGE *magImage, *grvImage; {
	...
	double *L, *M, *N, *Lptr, *Mptr, *Nptr, *Limg, *Mimg, *Nimg, *Liptr, *Miptr,
			*Niptr;
	...
	double reminc, remdec, susX, susY, susZ, gvar, ddir, dip, pitch, sus1, sus2,
			sus3, density;
	...
	if (variable) { /* create resample buffers if using varying cube sizes */
	...
	if (upbreal == NULL)
		return (exit_memory("upbreal"));
	upbimg = (double*) create1DArray(vpxy, sizeof(double));
	if (upbimg == NULL)
		return (exit_memory("upbimg"));
	...
	if (drapedCalc || xyzCalc) {
		...
		if (drapedCalc) /* grab the toplogy data */
		{
			...
			if (L == NULL)
				return (exit_memory("Topography Buffer1"));
			M = (double*) create1DArray(nx * dy, sizeof(double));
			if (M == NULL)
				return (exit_memory("Topography Buffer2"));
...
} else /* compute the variable earth field */
{
	...
	for (k = 0; k < nz; k++) /* compute field for each voxel */
	{
		for (j = 0; j < ny; j++) {
			for (i = 0; i < nx; i++) {
				tmp++; /* count of voxels */
				CSIROgetFieldAtLocation(&geoFizz, grid * (double) i,
						grid * (double) j, grid * (double) k, &inclination,
						&declination, &intensity);
		...
		for (j = 0; j < layerDimensions[layer][1]; j++) {
			...
			for (i = 0; i < layerDimensions[layer][0]; i++) {
				...
				if (aniCalc) {
					...
					if (indexCalc) {
						if (index < 0) {
							sus1 = sus2 = sus3 = dip = ddir = pitch = 0.0;
						} else {
...
for (k = 0; k < nz; k++) /* from top to bottom */
{
		...
		for (j = 0; j < layerDimensions[layer][0]; j++) {
			...
			if (variable) {
				if (layerDimensions[layer][0] != nx) {
					memcpy(buffer, Lst,
							layerDimensions[layer][0] * sizeof(double));
	...
	if (variable) {
		if (layerDimensions[layer][1] != ny) {
			resample_columns(L, buffer, buffer2, nx, vpx,
					layerDimensions[layer][1], ny);
			resample_columns(M, buffer, buffer2, nx, vpx,
					layerDimensions[layer][1], ny);
			resample_columns(N, buffer, buffer2, nx, vpx,
					layerDimensions[layer][1], ny);
		}
	}
	...
	pad_up(L, wrap_options.type, wrap_options.fence, Lmean, tmp, 1.0, vpx, vpy,
			nx, ny);
	pad_up(M, wrap_options.type, wrap_options.fence, Mmean, 1.0, tmp, vpx, vpy,
			nx, ny);
	pad_up(N, wrap_options.type, wrap_options.fence, Nmean, 1.0, 1.0, vpx, vpy,
			nx, ny);
	...
	{ /* The convolution in the spectral domain to calculate magnetics */
		...
		for (i = 0; i < vpy; i++) {
			...
			for (j = 0; j < vpx; j++) {
				...
				if (calcMag == 3) { /* l, m, n are earth mag field unit vectors */
					lumv = l * u + m * v;
					n_sq = n * uv_sqrt;
				}
	...
	if (calcMag < 3) { /* ******************************************** */
		...
		{
			up_back_add(Z, L, Limg, mXreal, upbreal, upbimg, ht_index, npoints,
					vpx, vpy, vpxy, uvSqrtArray);
			up_back_add(Z, M, Mimg, mYreal, upbreal, upbimg, ht_index, npoints,
					vpx, vpy, vpxy, uvSqrtArray);
		} else {
	...
	if (drapedCalc || xyzCalc)
		up_back_add(Z, N, Nimg, mZreal, upbreal, upbimg, ht_index, npoints, vpx,
				vpy, vpxy, uvSqrtArray);
	else {
...
if (outputFilename) {
		...
		if (DataBase == 0) {
			if (calcMag > 1) {
				...
				if (!(magFile = fopen(outputFilename, "w"))) {
					if (batchExecution)
						fprintf(stderr,
								"Cannot write NODDY magnetics file: %s\n",
								outputFilename);
					else
				...
				CSIROWriteMap(nx, ny, k, mXreal, magFile, nz, height,
						viewOptions, options, "333");
				fclose(magFile);
				...
				CSIROWriteMap(nx, ny, k, mYreal, magFile, nz, height,
						viewOptions, options, "333");
				fclose(magFile);
                ...
                int nnn;
		if (DataBase == 1)
			//CSIROUploadMap(nx, ny, k, mZreal, magFile, nz, height, viewOptions,
					//options, "333");
             nnn=1;
		else {
			CSIROWriteMap(nx, ny, k, mZreal, magFile, nz, height, viewOptions,
					options, "333");
        nnn=1;
			fclose(magFile);
		}
	} else /* xyz points */
	{
		...
		{ /* At end of this 'line2' has column headings and 'line' the first data values */
			...
			while (!xyzLineFound && fgets(line, 200, pointFile)) { /* Use the first line with a number at the start as a sample */
				sscanf(line, "%s", word);
				if (isdigit(word[0])) {
					xyzLineFound = TRUE;
...
if (densityCalc) {
	...
	for (k = 0; k < nz; k++) /* from top to bottom */
	{
			...
			for (j = 0; j < layerDimensions[layer][0]; j++) {
				...
				if (variable) {
					if (layerDimensions[layer][0] != nx) {
						memcpy(buffer, gst,
								layerDimensions[layer][0] * sizeof(double));
						rebin_vec(buffer, gst, layerDimensions[layer][0], nx, 1,
								1);
						grptr = gst + nx;
					}
				}
		...
		if (variable) {
			if (layerDimensions[layer][1] != ny)
				resample_columns(Greal, buffer, buffer2, nx, vpx,
						layerDimensions[layer][1], ny);
		}
		/* **************** */
		/* pad up the array */
		pad_up(Greal, wrap_options.type, wrap_options.fence, Gmean, 1.0, 1.0,
				vpx, vpy, nx, ny);
		...
		if (drapedCalc || xyzCalc) /* Convert back to spatial domain from spectral */
		{
			up_back_add(Z, GX, GXimg, gXreal, upbreal, upbimg, ht_index,
					npoints, vpx, vpy, vpxy, uvSqrtArray);
			up_back_add(Z, GY, GYimg, gYreal, upbreal, upbimg, ht_index,
					npoints, vpx, vpy, vpxy, uvSqrtArray);
			up_back_add(Z, GZ, GZimg, gZreal, upbreal, upbimg, ht_index,
					npoints, vpx, vpy, vpxy, uvSqrtArray);
		} else {
			Z2 = fabs(height - Z);
	...
	if (outputFilename) { /* *************************** */
		if (heightCalc || drapedCalc) /* Write out the gravity image */
		{
			...
			if (DataBase == 0) {
				if (calcMag > 1) {
					...
					CSIROWriteMap(nx, ny, k, gXreal, grvFile, nz, height,
							viewOptions, options, "444");
					fclose(grvFile);
					...
					CSIROWriteMap(nx, ny, k, gYreal, grvFile, nz, height,
							viewOptions, options, "444");
					fclose(grvFile);
			...
			if (DataBase == 1)
                 nnn=1;
				//CSIROUploadMap(nx, ny, k, gZreal, grvFile, nz, height,
				//		viewOptions, options, "444");
			else {
				CSIROWriteMap(nx, ny, k, gZreal, grvFile, nz, height,
						viewOptions, options, "444");
				fclose(grvFile);
			}
		} else /* xyz points */
		{
			...
			if (pointFile) { /* At end of this 'line2' has column headings and 'line' the first data values */
				int firstTime = TRUE;
