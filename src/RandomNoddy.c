...
//extern PGconn *conn;  //Postgres connection pointer
...
int RandomNoddy(char *output , int DataBase) {
 ...
 //	PGresult *res;
	...
	sprintf(time_stamp, "%ld-%ld", start.tv_sec, start.tv_usec);
...
int loadRandomFold(options)
	...
	//if(options->type == FOURIER_FOLD)
	//	loadRandomProfile(options);
	return (TRUE);
}

int loadRandomProfile (options)
PROFILE_OPTIONS *options;
{
   ...
   options->graphLength=200;
   options->minX=0;
   options->maxX=4000.0;

   options->minYScale=-1000.0;
   options->maxYScale= 1000.0;
   options->scaleOrigin=0.0;
   options->minYReplace=-1000.0;
   options->maxYReplace= 1000.0;
...
int loadRandomProperties(layer, options)
	...
	if (layer == 0)
		...
		options->height = 50
				+ (950.0 * xrshr128p_next_double(&state));
...
int report_Random_status(filename, DataBase)
	char *filename;
	int DataBase;
	 {
	FILE *out;
	...
	//	upload_Random_icon_positions(query);
	...
	} else
		{
					...
					printf(" Filename DataBase Random= %s %d %d",filename, DataBase, Random);
		}
...
int report_Random_proj_opts(out)
	FILE *out; {
	fprintf(out, "\n#ProjectOptions\n");
