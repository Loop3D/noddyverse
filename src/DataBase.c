...
int upload_Random_window_positions(query)
	char *query; {
	register int winNum;
...
#endif
{
	   ...
	   for (z = 0;z<numLayers;z++)
	   {
		   ...
		   {                  /* point to a single line of data */
		      ...
		      for (x = 0; x < layerDimensions[0][0]; x++, dataPtr+=dataIncrement)
		      {
		    	  ...
		    	  //fprintf(fo,"%c",*((short *) dataPtr));
		    	  //fflush(fo);
		    	  transposed[z][199-x][y]= *((short *) dataPtr);
	...
	for (z = 0;z<numLayers;z++) {
		upload2DBlock(fo, transposed[199-z], layerDimensions[z][1],
...
#endif
{
	int x, y, n,dataIncrement;
	char *dataPtr;
	char local_str[405],target_str[201];
	...
	for (x = 0; x < nx; x++) {
        ...
        for(n=0;n<200;n++)
        {
        	target_str[n]=data[x][2*n];
        	//printf("%d\t%d\t%c\n",x,n,target_str[n]);
        }
        target_str[200] = '\0';
        //fprintf(fo,"%d\t%s\n",x,target_str);
		strcat(query, target_str);
